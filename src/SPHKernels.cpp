...
void SPHKernels::density(Particle* p1, Particle* p2) {
  ...
  if (kernel_radius_squared > distanceSquared) {
    float extra_density = (p1->getMass()) * poly_6_const *
                          pow((kernel_radius_squared - distanceSquared), 3);
...
void SPHKernels::viscosity(Particle* p1, Particle* p2) {
  ...
  if (kernel_radius_squared > distanceSquared) {
    visc_force = visc_const * p2->getMass() *
                 ((p2->getU() - p1->getU()) / p2->getDensity()) *
                 (kernel_radius - distance);
    du = visc_force;
    visc_force = visc_const * p2->getMass() *
                 ((p2->getV() - p1->getV()) / p2->getDensity()) *
                 (kernel_radius - distance);
    dv = visc_force;
    p1->addForce(dv, du);
    visc_force = visc_const * p1->getMass() *
                 ((p1->getU() - p2->getU()) / p1->getDensity()) *
                 (kernel_radius - distance);
    du = visc_force;
    visc_force = visc_const * p1->getMass() *
                 ((p1->getV() - p2->getV()) / p1->getDensity()) *
                 (kernel_radius - distance);
    dv = visc_force;
    p2->addForce(dv, du);
  }
...
void SPHKernels::pressure(Particle* p1, Particle* p2) {
  ...
  if (kernel_radius_squared > distanceSquared && distanceSquared != 0) {
    float pressure_force =
        ((p1->getPressure() + p2->getPressure()) / (2 * p2->getDensity())) *
        24 / (pi * pow(kernel_radius, 5)) *
        (sqrt(kernel_radius_squared) - sqrt(distanceSquared));
    float du = pressure_force * (xDistance / sqrt(distanceSquared));
    float dv = pressure_force * (yDistance / sqrt(distanceSquared));
    p1->addForce(du, dv);
    pressure_force =
        ((p2->getPressure() + p1->getPressure()) / (2 * p1->getDensity())) *
        24 / (pi * pow(kernel_radius, 5)) *
        (sqrt(kernel_radius_squared) - sqrt(distanceSquared));
...
void SPHKernels::surface_tension(Particle* p1, Particle* p2) {
  ...
  float distanceSquared = xDistance * xDistance + yDistance * yDistance;
  if (kernel_radius_squared > distanceSquared && distanceSquared != 0) {
    float n = (p1->getMass() / p1->getDensity()) * poly_6_grad_const *
              pow((kernel_radius_squared - distanceSquared), 2);
    float surface_force =
        st * (p1->getMass() / p1->getDensity()) * poly_6_lap_const *
        (distanceSquared - kernel_radius_squared) *
        (5 * distanceSquared - kernel_radius_squared) * n / std::abs(n);
    float du = surface_force * (xDistance / sqrt(distanceSquared));
    float dv = surface_force * (yDistance / sqrt(distanceSquared));
    p1->addForce(du, dv);
    n = (p2->getMass() / p2->getDensity()) * poly_6_grad_const *
        pow((kernel_radius_squared - distanceSquared), 2);
    surface_force = st * (p2->getMass() / p2->getDensity()) * poly_6_lap_const *
                    (distanceSquared - kernel_radius_squared) *
                    (5 * distanceSquared - kernel_radius_squared) * n /
                    std::abs(n);
